...
@pytest.mark.asyncio
async def test_get_reviews_for_books(mock_db, mock_review_record):
    repo = ReviewRepository(mock_db)
...
@pytest.mark.asyncio
async def test_get_review_and_book_by_review_id(mock_db, mock_review_record):
    repo = ReviewRepository(mock_db)
...
@pytest.mark.asyncio
async def test_get_most_recent_book_reviews(mock_db, mock_review_record):
    repo = ReviewRepository(mock_db)
...
@pytest.mark.asyncio
async def test_get_reviews_by_book_id(mock_db, mock_review_record):
    repo = ReviewRepository(mock_db)
